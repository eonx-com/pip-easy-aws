...
class EasyCloudFormation:
    ...
    def check_stack_drift(stack_name, max_attempts=3, attempt_interval=3):
        ...
        :return: bool
        """
        EasyLog.trace('Checking drift on stack: {stack_name}...'.format(stack_name=stack_name))

        drift_status = None
        attempt_count = 0
        response = {}

        EasyLog.debug('Request drift detection...')
        ...
        while drift_status is None or drift_status == 'DETECTION_IN_PROGRESS' and attempt_count <= max_attempts:
            EasyLog.debug('Polling drift detection status...')
            ...
            if 'DetectionStatus' not in response:
                EasyLog.error('Could not find expected `DetectionStatus` key in response from AWS CloudFormation client')
                raise Exception(EasyCloudFormation.ERROR_INVALID_RESPONSE)

            drift_status = response['DetectionStatus']

            if drift_status == 'DETECTION_IN_PROGRESS':
                EasyLog.debug('Sleeping {attempt_interval} seconds...'.format(attempt_interval=attempt_interval))
                attempt_count += 1
                sleep(attempt_interval)

        EasyLog.debug('Finished polling for stack drift...')
        EasyLog.debug('Last detection status: {drift_status}'.format(drift_status=drift_status))
        ...
        if drift_status == 'DETECTION_FAILED':
            ...
            if 'DetectionStatusReason' not in response:
                EasyLog.error('Could not find expected `DetectionStatusReason` key in response from AWS CloudFormation client')
                raise Exception(EasyCloudFormation.ERROR_INVALID_RESPONSE)

            EasyLog.error('Drift detection failed: {reason}'.format(reason=response['DetectionStatusReason']))

            if 'Failed to detect drift on resource' not in response['DetectionStatusReason']:
                EasyLog.error('Drift detection failed for the following reason: {reason}'.format(reason=response['DetectionStatusReason']))
    ...
    def detect_stack_drift(stack_name) -> str:
        ...
        if stack_name not in stacks:
            EasyLog.error(EasyCloudFormation.ERROR_STACK_NOT_FOUND)
        ...
        try:
            ...
            EasyLog.debug('Requesting drift status check...')
            ...
            EasyLog.debug('Drift check started: {request_id}'.format(request_id=response['StackDriftDetectionId']))
            ...
            if 'Drift detection is already in progress for stack' in detect_exception.response['Error']['Message']:
                EasyLog.debug('Drift detection already in progress for this stack')
                ...
                if stack_name in EasyCloudFormation.__drift_request_ids__:
                    if EasyCloudFormation.__drift_request_ids__[stack_name] is not None:
                        ...
                        EasyLog.debug('Returning cached request ID: {request_id}'.format(request_id=EasyCloudFormation.__drift_request_ids__[stack_name]))
                ...
                EasyLog.error('Drift detection was already initiated by external process')
                raise Exception(EasyCloudFormation.ERROR_DRIFT_DETECTION_CONFLICT)

            # Something else went wrong
            EasyLog.error('An unexpected error occurred while checking for stack drift: {detect_exception}'.format(detect_exception=detect_exception))
            raise Exception(EasyCloudFormation.ERROR_DRIFT_DETECTION_FAILED)
